...
DATA_DIR = 'AAA_slide_output/show_0'
OUT_DIR = DATA_DIR+'_1_visualized'
...
for j in range(44):
    ...
    for i in trange(n):
        ...
        if nclass == max_n_class:
            ...
            # --- [新增] 保存真值 (GT) ---
            save_m_file(e, fname=f'm_GT_{i+j*n}.npy')
        ...
        if T_pred is not None:
            ...
            # --- [新增] 保存真值 (GT) ---
            save_m_file(T, fname=f'T_GT_{i+j*n}.npy')
        ...
        if v_pred is not None:
            ...
            # --- [新增] 保存真值 (GT) ---
            # 注意：v 这里已经是 tensor 或 numpy，如果 v 还是 tensor 需要先转 numpy
            # 假设 v 在进入此 block 前已经处理好，或者是 tensor
            if hasattr(v, 'numpy'): 
                v_npy = v.numpy()
            else:
                v_npy = v
            save_m_file(v_npy, fname=f'v_GT_{i+j*n}.npy')
        ...
        # --- [新增] 可视化 X (Texture) ---
        # 假设 X 是连续值，使用 visualize_T 进行可视化，或者你可以定义 visualize_X
        # 如果 X_pred 是 numpy 数组
        visualize_T(X_pred, fname=f'Xmap_pred_{i+j*n}.png', error=False, kind='pred')
        visualize_T(X_true, fname=f'Xmap_GT_{i+j*n}.png', error=False, kind='gt')
        
        # 如果你也想保存 X 的 npy 文件：
        save_m_file(X_pred, fname=f'X_pred_{i+j*n}.npy')
        save_m_file(X_true, fname=f'X_GT_{i+j*n}.npy')
