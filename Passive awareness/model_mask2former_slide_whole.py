...
from Mask2Former import TeXMask2Former
...
class SMPModel(pl.LightningModule):
    ...
    def __init__(self, args):
        ...
        # self.texnet = smp.FPN(encoder_name='SwinMAE', encoder_weights=None, in_channels=49, classes=num_out_channels)
        self.texnet = TeXMask2Former(
                 nclass=self.nclass, 
    ...
    def slide_inference(self, img):
        """
        对大图进行滑动窗口推理，并进行重叠区域平均投票 (Voting)。
        img: [1, C, H, W] (Batch Size 必须为 1)
        """
        batch, channel, h_img, w_img = img.shape
        h_crop, w_crop = self.crop_size
        h_stride, w_stride = self.stride
        
        # 1. 准备画布 (Canvas)
        # 输出通道 = eMap(30) + T(1 if train_T) + v(2 if train_v)
        total_out_ch = self.nclass
        if self.train_T: total_out_ch += 1
        if self.train_v: total_out_ch += 2
        
        # preds: 累加预测值的画布
        preds = torch.zeros((batch, total_out_ch, h_img, w_img), device=img.device)
        # count_mat: 记录每个像素被预测了多少次 (用于求平均)
        count_mat = torch.zeros((batch, 1, h_img, w_img), device=img.device)
        ...
        # 简单的 grid 生成逻辑
        for h in range(0, h_img - h_crop + 1, h_stride):
            h_grids.append(h)
        if (h_img - h_crop) % h_stride != 0: # 处理边缘
            h_grids.append(h_img - h_crop)
            
        for w in range(0, w_img - w_crop + 1, w_stride):
            w_grids.append(w)
        if (w_img - w_crop) % w_stride != 0: # 处理边缘
            w_grids.append(w_img - w_crop)

        # 3. 开始滑动循环
        for h_idx in h_grids:
            for w_idx in w_grids:
                # crop 图片
                img_crop = img[:, :, h_idx : h_idx + h_crop, w_idx : w_idx + w_crop]
                
                # 模型推理
                with torch.no_grad():
                    pred_crop = self.texnet(img_crop)
                
                # 累加到画布
                preds[:, :, h_idx : h_idx + h_crop, w_idx : w_idx + w_crop] += pred_crop
                count_mat[:, :, h_idx : h_idx + h_crop, w_idx : w_idx + w_crop] += 1.0

        # 4. 求平均 (Voting)
        assert (count_mat == 0).sum() == 0, "Error: Some pixels were not covered by sliding window!"
    ...
    def configure_optimizers(self):
        base_lr = 0.0001  # 对应配置中的 0.0001
        base_wd = 0.05  # 对应配置中的 0.05
        
        # 2. 构建细粒度的参数组 (Paramwise Config)
        params_group = []
        
        # 遍历模型的所有参数
        for name, param in self.texnet.named_parameters():
            ...
            # --- 规则 A: Backbone 学习率设为 0.1 倍 ---
            # 对应 config: 'backbone': dict(lr_mult=0.1, decay_mult=1.0)
            if 'backbone' in name:
                p_lr = base_lr * 0.1
            
            # --- 规则 B: 特殊层的 Weight Decay 设为 0 ---
            # 1. 对应 config: query_embed, query_feat, level_embed 的 decay_mult=0.0
            if any(key in name for key in ['query_embed', 'query_feat', 'level_embed']):
                p_wd = 0.0
            
            # 2. 对应 config: norm_decay_mult=0.0
            # 通常 Norm 层的 weight/bias 和卷积/全连接的 bias 都是 1 维向量
            # 从你提供的参数列表看，norm.weight 都是 [128] 等一维形状，符合此规则
            elif param.ndim == 1:
                p_wd = 0.0
    ...
    def validation_step(self, batch, batch_idx):
        self.e_val_list = self.e_val_list.to(self.device)
        S_beta, img, tgt = batch
        # Dataset 返回的是全尺寸图片，batch_size=1
        
        t, e, v = tgt
        # 注意：这里 e, t, v 也是全尺寸的 Ground Truth
        ...
        # --- 调用滑动窗口推理 ---
        # 现在的 pred 是拼好的全尺寸预测图 [1, C_out, H, W]
        pred = self.slide_inference(img)
        ...
        num_v_channels = 2 # 假设 v 是 2 通道
        if v.ndim == 4: num_v_channels = v.shape[1]

        # 初始化 Loss
        loss_T = torch.tensor([0.], device=self.device)
        loss_v = torch.tensor([0.], device=self.device)
        ...
        if self.train_v:
            ...
            v_pred = self.softmax(v_pred)
            v_pred_log = torch.log(v_pred + 1e-6) # 加 eps 防止 nan
            loss_v = self.v_map_criterion(v_pred_log, v)
        else:
            v_pred = v
        ...
        # 计算 IoU
        e_pred_idx = torch.argmax(e_pred, 1, keepdim=False).type(torch.long)
        val_iou = self.miou(e_pred_idx, e)

        # --- 无监督 Loss (全图计算) ---
        # 这一步会生成全尺寸的 S_pred
        loss_S, S_pred, corr_score = self.unsupervised_S_pred_loss(
            img, T_pred, e_pred_idx, v_pred, S_beta,
            no_grad=True, calc_score=self.calc_score
        )
        
        # 汇总 Loss
        loss = loss_e + loss_T + loss_v 

        # Log
        self.log('val_loss', loss, on_step=False, on_epoch=True, sync_dist=True)
        self.log('val_miou', val_iou, on_step=False, on_epoch=True, sync_dist=True)
        self.log('val_loss_S', loss_S, on_step=False, on_epoch=True, sync_dist=True)
        ...
        # 使用 .cpu() 将 Tensor 移至 CPU 再保存，避免占用显存和后续加载时的设备冲突
        # img, t, e, v 是 Ground Truth
        # pred 是模型原始输出, S_pred 是物理重建的光谱图
        torch.save(t.cpu(), f'{self.checkpoint_dir}/val_T_{batch_idx}.pt')
        torch.save(e.cpu(), f'{self.checkpoint_dir}/val_e_{batch_idx}.pt')
        torch.save(v.cpu(), f'{self.checkpoint_dir}/val_v_{batch_idx}.pt')
        torch.save(pred.cpu(), f'{self.checkpoint_dir}/val_pred_{batch_idx}.pt')
        torch.save(S_pred.cpu(), f'{self.checkpoint_dir}/val_S_pred_{batch_idx}.pt')
        torch.save(img.cpu(), f'{self.checkpoint_dir}/val_S_true_{batch_idx}.pt')
        
        print(f"Saved validation tensors for batch {batch_idx} to {self.checkpoint_dir}")
