...
class ATM(nn.Module):    
    def __init__(self, W, H, channel):
        super(ATM, self).__init__()
        self.W = W
        self.H = H
        self.channel = channel
        self.weight1 = torch.zeros(channel,self.H,self.W)
        self.weight1 = torch.nn.Parameter(self.weight1)
        self.weight2 = torch.zeros(channel,self.H,self.W)
        self.weight2 = torch.nn.Parameter(self.weight2)
        self.d = torch.ones(channel,self.H,self.W)
        self.d = torch.nn.Parameter(self.d)
 
    def forward(self, input):
        input_ = input
        y=torch.mul(input_,torch.exp((self.weight1+self.weight2)*self.d))
        return y
