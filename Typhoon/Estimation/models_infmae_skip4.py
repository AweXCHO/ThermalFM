...
# MCMAE: https://github.com/Alpha-VL/ConvMAE
# --------------------------------------------------------
...
class MaskedAutoencoderInfMAE(nn.Module):
    ...
    def _init_weights(self):
        checkpoint_path = '/mnt/dqdisk/CODE/InfMAE.pth'
    ...
    def forward_decoder(self, x, ids_restore):
        ...
        x_ = x
        # 插入的间隔和插入的次数
        insert_interval = 3  # 每隔一个向量插入48个向量
        ...
        # 初始化新的结果张量
        result = torch.empty(*result_shape).cuda()

        # 插入 mask_tokens 到 x_ 中
        for i in range(insert_count):
            ...
            if start_idx == 0:
                ...
                result[:, start_idx + 1:end_idx + 1, :] = mask_tokens[:, i * insert_interval:(i + 1) * insert_interval,:]
            else:
                result[:, start_idx:start_idx + 1, :] = x_[:, i, :].unsqueeze(1)
                result[:, start_idx + 1:end_idx + 1, :] = mask_tokens[:, i * insert_interval:(i + 1) * insert_interval,:]

        x = torch.gather(result, dim=1, index=ids_restore.unsqueeze(-1).repeat(1, 1, x.shape[2]))  # unshuffle
