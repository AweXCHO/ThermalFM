...
from swin_unet import PatchEmbedding, PatchExpanding
# todo: 这里改成了未加物理模块的
# from swin_unet import BasicBlock, BasicBlockUp
#只修改了这里，用了自己d定义的swin-block
...
class SwinMAE(nn.Module):
    ...
    def __init__(self, img_size: int = 224, patch_size: int = 4, mask_ratio: float = 0.75, in_chans: int = 3,
                 ...
                 depths: tuple = (2, 2, 18, 2), embed_dim: int = 128, num_heads: tuple = (4, 8, 16, 32),
        ...
        self.low_pass_filter = self._generate_butterworth_low_pass_filter(40, 2, img_size, img_size)
        self.radiation_module = RadiationModule(dim=in_chans)
        self.TIM_module = TIM(H=img_size, W=img_size)
        self.ATM_module = ATM(H=img_size, W=img_size,channel=in_chans)
    ...
    def forward_encoder(self, x):
        ...
        # print("x_min:", x.min())
        #x = x + radiation
        x = self.ATM_module(x) # ATM模块，大气传输
        ...
        # tim = self.TIM_module(x) # TIM模块，热惯性模块
        # 过一个MLP，将结果加在decoder中，通过拼接的方式
    ...
    def forward_loss(self, imgs, pred, mask):
        ...
        self.low_pass_filter = self.low_pass_filter.to(imgs.device)
        ...
        #原来的代码
        target = self.patchify(low_pass_imgs)
    ...
    def _generate_butterworth_low_pass_filter(self,cutoff, n, height=224, width=224):
        """Generate a Butterworth low pass filter using PyTorch.
        ...
        Returns:
            torch.Tensor: Butterworth low pass filter.
        """
        h, w = height, width
        ...
        # Calculate Butterworth filter values
        butterworth_filter = 1 / (1 + (distance / cutoff)**(2 * n))

        # Create a 3-channel filter
        butterworth_filter = butterworth_filter.unsqueeze(0).repeat(3, 1, 1)

        return butterworth_filter


class SwinMAE_ablaion(nn.Module):
    """
    Masked Auto Encoder with Swin Transformer backbone
    """
    ...
    def __init__(self, img_size: int = 224, patch_size: int = 4, mask_ratio: float = 0.75, in_chans: int = 3,
                 ...
                 window_size: int = 7, qkv_bias: bool = True, mlp_ratio: float = 4.,
                 drop_path_rate: float = 0.1, drop_rate: float = 0., attn_drop_rate: float = 0.,
                 norm_layer=nn.LayerNorm, patch_norm: bool = True, atm=True):
        super().__init__()
        self.mask_ratio = mask_ratio
        assert img_size % patch_size == 0
        self.num_patches = (img_size // patch_size) ** 2
